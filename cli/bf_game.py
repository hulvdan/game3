...
def _process_glib(genline, glib) -> None:
  ...
  team_type_2_value: dict[str, int] = {x["type"]: x["enum__value"] for x in glib["teams"]}
        ...
        doors.append(
          {
            ...
            "direction": transpose_direction(int(x.field("Direction").split("_", 1)[-1])),
          }
        )
  ...
  ## Tags
  entity_2_tag_required_fields: dict[str, dict[str, list[str]]] = defaultdict(defaultdict)
  ...
  for field_name, type_name in table_field_name_type_pairs:
    ...
    for t in types:
      assert t.upper() == t, "{}. {}. `type` fields must be in CONSTANT_CASE".format(
        field_name, t
      )
    ...
    assert bf.all_are_none(overridden_values) or bf.all_are_not_none(overridden_values), (
