...
func set_transform(transform: Transform2D):
...
func line(from: Vector2, to: Vector2, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.draw_line_2d(from, to)
	EditorImmediateGizmos.end_draw_2d(color)


func line_strip(points: Array[Vector2], color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.points_2d.append_array(points)
	EditorImmediateGizmos.end_draw_2d(color)


func line_polygon(points: Array[Vector2], color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.points_2d.append_array(points)
	if (points.size() <= 0):
		return
...
func line_arc(center: Vector2, startPoint: Vector2, radians: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.draw_arc_2d(center, startPoint, radians)
	EditorImmediateGizmos.end_draw_2d(color)


func line_circle(center: Vector2, radius: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.draw_arc_2d(center, Vector2.UP * radius, TAU)
	EditorImmediateGizmos.end_draw_2d(color)


func line_capsule(center: Vector2, radius: float, height: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	height -= radius * 2
	if (height < 0):
...
func line_rect(center: Vector2, size: Vector2, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
...
func line_square(center: Vector2, size: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	line_rect(center, Vector2.ONE * size, color)

##########################################################################


func draw_text(text: String, position: Vector2, hAlign: HorizontalAlignment = HORIZONTAL_ALIGNMENT_LEFT, vAlign: VerticalAlignment = VERTICAL_ALIGNMENT_BOTTOM, height: float = 0.25):
