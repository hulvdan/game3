...
func set_required_selection(node: Node):
...
func line(from: Vector3, to: Vector3, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.draw_line_3d(from, to)
	EditorImmediateGizmos.end_draw_3d(color)


func line_strip(points: Array[Vector3], color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.points_3d.append_array(points)
	EditorImmediateGizmos.end_draw_3d(color)


func line_polygon(points: Array[Vector3], color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.points_3d.append_array(points)
	if (points.size() <= 0):
		return
...
func line_arc(center: Vector3, axis: Vector3, startPoint: Vector3, radians: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	EditorImmediateGizmos.draw_arc_3d(center, axis, startPoint, radians)
	EditorImmediateGizmos.end_draw_3d(color)


func line_circle(center: Vector3, axis: Vector3, radius: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
...
func line_sphere(center: Vector3, radius: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
...
func line_capsule(center: Vector3, radius: float, height: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	height -= radius * 2
	if (height < 0):
...
func line_cuboid(center: Vector3, radius: Vector3, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
...
func line_cube(center: Vector3, radius: float, color: Color = EditorImmediateGizmos.gizmo_default_color) -> void:
	line_cuboid(center, Vector3.ONE * radius, color)

##########################################################################


func draw_text(text: String, position: Vector3, hAlign: HorizontalAlignment = HORIZONTAL_ALIGNMENT_LEFT, vAlign: VerticalAlignment = VERTICAL_ALIGNMENT_BOTTOM, height: float = 0.25):
