...
@onready var crazy_banner_w_node: OptionButton = $MarginContainer/ScrollContainer/VFlowContainer/VBoxContainer/CrazyBannerSize/crazy_banner_w
@onready var crazy_banner_h_node: OptionButton = $MarginContainer/ScrollContainer/VFlowContainer/VBoxContainer/CrazyBannerSize/crazy_banner_h
@onready var is_archive_node: CheckBox = $"MarginContainer/ScrollContainer/VFlowContainer/VBoxContainer2/HBoxContainer2/Is Archive"
@onready var yandex_archive_name_node: LineEdit = $MarginContainer/ScrollContainer/VFlowContainer/VBoxContainer2/HBoxContainer/yandex_archive_name
@onready var crazy_banner_pos_node: OptionButton = $MarginContainer/ScrollContainer/VFlowContainer/VBoxContainer/CrazyBannerPos/crazy_banner_pos
...
func _load_config() -> void:
	...
	is_archive_node.button_pressed = is_archive
	yandex_archive_name_node.text = archive_name
	var id_pos:int
	match crazy_banner_pos:
		...
		"right":
			id_pos = 3
	crazy_banner_pos_node.select(id_pos)
	for i in range(crazy_banner_w_node.item_count):
		if crazy_banner_w == crazy_banner_w_node.get_item_text(i):
			crazy_banner_w_node.select(i)
	for i in range(crazy_banner_h_node.item_count):
		if crazy_banner_h == crazy_banner_h_node.get_item_text(i):
			crazy_banner_h_node.select(i)
...
func _on_crazy_banner_w_item_selected(index):
	crazy_banner_w = crazy_banner_w_node.get_item_text(index)
	main_scene_data_change.emit()
	_save_config()


func _on_crazy_banner_h_item_selected(index):
	crazy_banner_h = crazy_banner_h_node.get_item_text(index)
	main_scene_data_change.emit()
	_save_config()
