...
3. **Default Validations:** Godot Doctor also has a few default validations built-in, such as checking whether an object is valid (aka not null), or whether strings are non-mepty. If you have `use_default_validations` enabled in Godot Doctor's settings, these will automatically run, regardless of whether you have a `_get_validation_conditions()` function attached to the script. Note that these default validation checks do _not_ run on nodes that do not have a script attached.
...
1. **`my_string` is empty:** The default value `""` fails the default validation check.
