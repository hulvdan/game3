...
func _init() -> void:
	...
	titlebar_normal = (
		plugin.get_editor_interface().get_base_control().get_theme_stylebox(&"titlebar", &"GraphNode") \
		.duplicate()
	)
	...
	panel_normal = (
		plugin.get_editor_interface().get_base_control().get_theme_stylebox(&"panel", &"GraphNode").duplicate()
	)
	panel_success = (
		plugin.get_editor_interface().get_base_control().get_theme_stylebox(&"panel_selected", &"GraphNode").duplicate()
	)
